...
class Entity {
  ...
  constructItem = (name, room) => {
    const itemData = this.World.managers.DataManager.getFrom(`items/props/${name}`)
    ...
    if(this.World.managers.DataManager.hasDescription('item', name))
  ...
  addTag = (tagName, id) => {
      ...
      catch(error){/*ignore search already created error*/}
